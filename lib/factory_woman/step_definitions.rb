module FactoryWomanStepHelpers
    ...
    class AssociationManager
      ...
      def association_instance
        ...
        if attributes_hash = nested_attribute_hash
          ...
          FactoryWoman.create(association.factory, attributes_hash.attributes)
        end
      end
      ...
      def factory
        FactoryWoman.factory_by_name(association.factory)
      end
...
World(FactoryWomanStepHelpers)

FactoryWoman.factories.each do |factory|
  factory.compile
  factory.human_names.each do |human_name|
    Given /^the following (?:#{human_name}|#{human_name.pluralize}) exists?:$/i do |table|
      table.hashes.each do |human_hash|
        ...
        FactoryWoman.create(factory.name, attributes)
      end
    end

    Given /^an? #{human_name} exists$/i do
      FactoryWoman.create(factory.name)
    end

    Given /^(\d+) #{human_name.pluralize} exist$/i do |count|
      FactoryWoman.create_list(factory.name, count.to_i)
    end
    ...
    if factory.build_class.respond_to?(:columns)
      factory.build_class.columns.each do |column|
        ...
        Given /^an? #{human_name} exists with an? #{human_column_name} of "([^"]*)"$/i do |value|
          FactoryWoman.create(factory.name, column.name => value)
        end

        Given /^(\d+) #{human_name.pluralize} exist with an? #{human_column_name} of "([^"]*)"$/i do |count, value|
          FactoryWoman.create_list(factory.name, count.to_i, column.name => value)
        end
      end
    end
  end
