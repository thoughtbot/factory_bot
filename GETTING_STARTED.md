...
which factory_girl will constantize later, once you start building objects:
...
`Marshal.dump`, since factory_girl defines singleton methods on these objects.
...
[this blog post](https://robots.thoughtbot.com/deprecating-static-attributes-in-factory_girl-4-11).
...
If your attributes conflict with existing methods or reserved words (all methods in the [DefinitionProxy](https://github.com/thoughtbot/factory_girl/blob/master/lib/factory_girl/definition_proxy.rb) class) you can define them with `add_attribute`.
...
```ruby
FactoryGirl.use_parent_strategy = false
...
Sequences can also be rewound with `FactoryGirl.rewind_sequences`:
