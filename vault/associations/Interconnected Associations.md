...
## Referencing the `instance` Being Constructed

That said, some more complex, interconnected relationships can be built in factory\_bot using inline associations with reference to the `instance` being built.
...
> [!NOTE]
> This approach works with `build`, `build_stubbed`, and `create`, but the associations will return `nil` when using `attributes_for`.

> [!NOTE]
> If you assign any attributes inside a custom `initialize_with` (e.g. `initialize_with { new(**attributes) }`), those attributes should not refer to `instance`, since it will be `nil`.
