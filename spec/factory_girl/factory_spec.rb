...
describe FactoryWoman::Factory do
  before do
    @name    = :user
  ...
  it "passes a custom creation block" do
    ...
    FactoryWoman::Proxy::Build.stubs(:new => proxy)
    block = lambda {}
    factory = FactoryWoman::Factory.new(:object)
    factory.to_create(&block)

    factory.run(FactoryWoman::Proxy::Build, {})

    proxy.should have_received(:result).with(block)
  end

  it "returns associations" do
    factory = FactoryWoman::Factory.new(:post)
    FactoryWoman.register_factory(FactoryWoman::Factory.new(:admin))
    factory.declare_attribute(FactoryWoman::Declaration::Association.new(:author, {}))
    factory.declare_attribute(FactoryWoman::Declaration::Association.new(:editor, {}))
    factory.declare_attribute(FactoryWoman::Declaration::Implicit.new(:admin, factory))
  ...
  describe "when overriding generated attributes with a hash" do
    ...
    it "returns the overridden value in the generated attributes" do
      declaration = FactoryWoman::Declaration::Static.new(@name, 'The price is wrong, Bob!')
      @factory.declare_attribute(declaration)
      result = @factory.run(FactoryWoman::Proxy::AttributesFor, @hash)
      result[@name].should == @value
    end

    it "does not call a lazy attribute block for an overridden attribute" do
      declaration = FactoryWoman::Declaration::Dynamic.new(@name, false, lambda { flunk })
      @factory.declare_attribute(declaration)
      @factory.run(FactoryWoman::Proxy::AttributesFor, @hash)
    end
  ...
  describe "overriding an attribute with an alias" do
    before do
      @factory.declare_attribute(FactoryWoman::Declaration::Static.new(:test, 'original'))
      Factory.alias(/(.*)_alias/, '\1')
      @result = @factory.run(FactoryWoman::Proxy::AttributesFor,
                             :test_alias => 'new')
    end
  ...
  it "creates a new factory using the class of the parent" do
    child = FactoryWoman::Factory.new(:child, :parent => @factory.name)
    child.compile
    child.build_class.should == @factory.build_class
  end

  it "creates a new factory while overriding the parent class" do
    child = FactoryWoman::Factory.new(:child, :class => String, :parent => @factory.name)
    child.compile
    child.build_class.should == String
  end
end

describe FactoryWoman::Factory, "when defined with a custom class" do
  subject           { FactoryWoman::Factory.new(:author, :class => Float) }
  its(:build_class) { should == Float }
end

describe FactoryWoman::Factory, "when defined with a class instead of a name" do
  let(:factory_class) { ArgumentError }
  let(:name)          { :argument_error }

  subject { FactoryWoman::Factory.new(factory_class) }

  its(:name)        { should == name }
  its(:build_class) { should == factory_class }
end

describe FactoryWoman::Factory, "when defined with a custom class name" do
  subject           { FactoryWoman::Factory.new(:author, :class => :argument_error) }
  its(:build_class) { should == ArgumentError }
end

describe FactoryWoman::Factory, "with a name ending in s" do
  let(:name)           { :business }
  let(:business_class) { Business }

  before  { define_class('Business') }
  subject { FactoryWoman::Factory.new(name) }

  its(:name)        { should == name }
  its(:build_class) { should == business_class }
end

describe FactoryWoman::Factory, "with a string for a name" do
  let(:name) { :string }
  subject    { FactoryWoman::Factory.new(name.to_s) }
  its(:name) { should == name }
end

describe FactoryWoman::Factory, "for namespaced class" do
  let(:name)           { :settings }
  ...
  context "with a namespaced class with Namespace::Class syntax" do
    subject { FactoryWoman::Factory.new(name, :class => "Admin::Settings") }
...
describe FactoryWoman::Factory do
  let(:factory_with_non_existent_strategy) do
    FactoryWoman::Factory.new(:object, :default_strategy => :nonexistent) { }
  end

  let(:factory_with_stub_strategy) do
    FactoryWoman::Factory.new(:object, :default_strategy => :stub)
  end

  before do
    define_class("User")
    define_class("Admin", User)
    FactoryWoman.register_factory(factory_with_stub_strategy)
  end
  ...
  describe "defining a child factory without setting default strategy" do
    subject { FactoryWoman::Factory.new(:other_object, :parent => factory_with_stub_strategy.name) }
  ...
  describe "defining a child factory with a default strategy" do
    subject { FactoryWoman::Factory.new(:other_object, :default_strategy => :build, :parent => factory_with_stub_strategy.name) }
    before  { subject.compile }
...
describe FactoryWoman::Factory, "human names" do
  context "factory name without underscores" do
    subject           { FactoryWoman::Factory.new(:user) }
    its(:names)       { should == [:user] }
    its(:human_names) { should == ["user"] }
  end

  context "factory name with underscores" do
    subject           { FactoryWoman::Factory.new(:happy_user) }
    its(:names)       { should == [:happy_user] }
    its(:human_names) { should == ["happy user"] }
  end

  context "factory name with big letters" do
    subject           { FactoryWoman::Factory.new(:LoL) }
    its(:names)       { should == [:LoL] }
    its(:human_names) { should == ["lol"] }
  end

  context "factory name with aliases" do
    subject           { FactoryWoman::Factory.new(:happy_user, :aliases => [:gleeful_user, :person]) }
    its(:names)       { should == [:happy_user, :gleeful_user, :person] }
    its(:human_names) { should == ["happy user", "gleeful user", "person"] }
  end
end

describe FactoryWoman::Factory, "running a factory" do
  subject              { FactoryWoman::Factory.new(:user) }
  let(:attribute)      { FactoryWoman::Attribute::Static.new(:name, "value", false) }
  let(:declaration)    { FactoryWoman::Declaration::Static.new(:name, "value", false) }
  ...
  before do
    ...
    FactoryWoman::Declaration::Static.stubs(:new => declaration)
  ...
  it "creates the right proxy using the build class when running" do
    subject.run(FactoryWoman::Proxy::Build, {})
    FactoryWoman::Proxy::Build.should have_received(:new).with(subject.build_class, [])
  end
  ...
  it "returns the result from the proxy when running" do
    subject.run(FactoryWoman::Proxy::Build, {}).should == "result"
    proxy.should have_received(:result).with(nil)
  end

  it "sets overrides once on the factory" do
    subject.run(FactoryWoman::Proxy::Build, { :name => "John Doe" })
  ...
  it "calls the block and returns the result" do
    ...
    subject.run(FactoryWoman::Proxy::Build, { }, &block)
