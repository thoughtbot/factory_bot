...
describe FactoryWoman::DefinitionProxy, "#add_attribute" do
  subject     { FactoryWoman::Definition.new }
  let(:proxy) { FactoryWoman::DefinitionProxy.new(subject) }

  it "raises if both a block and value are given" do
    expect {
      proxy.add_attribute(:something, "great") { "will raise!" }
    }.to raise_error(FactoryWoman::AttributeDefinitionError, "Both value and block given")
  end
...
describe FactoryWoman::DefinitionProxy, "#add_attribute when the proxy ignores attributes" do
  subject     { FactoryWoman::Definition.new }
  let(:proxy) { FactoryWoman::DefinitionProxy.new(subject, true) }
...
describe FactoryWoman::DefinitionProxy, "#ignore" do
  subject     { FactoryWoman::Definition.new }
  let(:proxy) { FactoryWoman::DefinitionProxy.new(subject) }
...
describe FactoryWoman::DefinitionProxy, "#method_missing" do
  subject     { FactoryWoman::Definition.new }
  let(:proxy) { FactoryWoman::DefinitionProxy.new(subject) }
...
describe FactoryWoman::DefinitionProxy, "#sequence" do
  subject     { FactoryWoman::Definition.new }
  let(:proxy) { FactoryWoman::DefinitionProxy.new(subject) }

  before      { FactoryWoman::Sequence.stubs(:new) }

  it "creates a new sequence starting at 1" do
    proxy.sequence(:great)
    FactoryWoman::Sequence.should have_received(:new).with(:great, 1)
  end

  it "creates a new sequence with an overridden starting vaue" do
    proxy.sequence(:great, "C")
    FactoryWoman::Sequence.should have_received(:new).with(:great, "C")
  end
  ...
  it "creates a new sequence with a block" do
    ...
    FactoryWoman::Sequence.should have_received(:new).with(:great, 1, &sequence_block)
  end
end

describe FactoryWoman::DefinitionProxy, "#association" do
  subject     { FactoryWoman::Definition.new }
  let(:proxy) { FactoryWoman::DefinitionProxy.new(subject) }
...
describe FactoryWoman::DefinitionProxy, "adding callbacks" do
  subject        { FactoryWoman::Definition.new }
  let(:proxy)    { FactoryWoman::DefinitionProxy.new(subject) }
...
describe FactoryWoman::DefinitionProxy, "#to_create" do
  subject     { FactoryWoman::Definition.new }
  let(:proxy) { FactoryWoman::DefinitionProxy.new(subject) }
...
describe FactoryWoman::DefinitionProxy, "#factory" do
  subject     { FactoryWoman::Definition.new }
  let(:proxy) { FactoryWoman::DefinitionProxy.new(subject) }
...
describe FactoryWoman::DefinitionProxy, "#trait" do
  subject     { FactoryWoman::Definition.new }
  let(:proxy) { FactoryWoman::DefinitionProxy.new(subject) }
