...
describe FactoryGirl::Proxy do
  ...
  it "calls get for a missing method" do
    ...
    subject.name.should eq "it's a name"
  end
  ...
  describe "when adding callbacks" do
    it "adds a callback" do
      ...
      subject.callbacks[:after_create].should eq [callback]
    end
    ...
    it "adds multiple callbacks of the same name" do
      ...
      subject.callbacks[:after_create].should eq [one, two]
    end
    ...
    it "adds multiple callbacks with different names" do
      ...
      subject.callbacks[:after_create].should eq [after_create]
      subject.callbacks[:after_build].should eq [after_build]
    end
  end
